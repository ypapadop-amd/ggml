...
// Exercises event_wait: recording and waiting on the same backend queue must be a no-op that does
// not deadlock, and subsequent work must still be correctly ordered behind the recorded point.
...
int run(ggml_backend_t backend) {
    ...
    // Same-queue device-side ordering: must return without blocking the CPU and without deadlock.
    ggml_backend_event_wait(backend, event.get());

    // A full synchronize is still required before the host can read results.
    ...
    std::cout << "event_wait ordered same-queue work correctly for " << N << " elements\n";
    return EXIT_SUCCESS;
}
