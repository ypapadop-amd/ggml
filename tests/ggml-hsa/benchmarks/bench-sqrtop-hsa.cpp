...
void bench_sqrtop(benchmark::State & state) {
    // initialize backend
    ...
    ggml_tensor * tensor_result = ggml_step(ctx, tensor_a);
...
#define SQRTOP_SHAPES(BENCH)                                                                          \
    BENCH->Args({500, 500, 1, 1})                                                                   \
        ->Args({14, 14, 16, 500})                                                                   \
        ->Args({28, 28, 8, 500})                                                                    \
        ->UseRealTime()

SQRTOP_SHAPES(BENCHMARK(bench_sqrtop<BackendType::HSA, float>));
