...
The ported kernel is single-tile and shape-specialized (mirrors
...
- Shape: **M = N = K = 64**, 2D, contiguous operands.

Any node not matching this profile is handled exactly as today (IRON only).

## Success criteria

1. `build_triton` produces `<name>.pdi` + `<name>_insts.bin` for the 64³ bf16
...
3. `test-mul-mat-hsa` runs a 64³ bf16 matmul on-device (`/dev/accel0` is
...
for exactly this 64³ bf16 shape, so they are copied verbatim and referenced via
...
- `tests/ggml-hsa/test-mul-mat-hsa.cpp` — add bf16 support to
  `fundamental_to_ggml_type`, parametrize dims, add a 64³ bf16 case with a
  tolerance check.
...
dtype == bf16, output dtype == f32, `ne0 == ne1 == 64` on both operands,
M == N == 64, and operands contiguous. The deferred `_compile` **re-checks**
...
- Add bf16 to `fundamental_to_ggml_type` (`GGML_TYPE_BF16`) and a bf16
  print/compare path.
- Parametrize M/N/K; add a 64³ bf16 case alongside the existing i16 path.
...
1. `build_triton` on a 64³ bf16 node emits both artifacts (compile-only,
   no device needed).
2. `ggml_op_mul_mat` returns Triton-first for 64³ bf16 and IRON-only otherwise
   (quick Python check).
...
- **Transform-script arch coverage:** verbatim scripts are tuned for 64³ bf16;
