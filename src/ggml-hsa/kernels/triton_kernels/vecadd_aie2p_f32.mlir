...
////////////////////////////////////////////////////////////////////////////////
// Transform Script for Vector Addition (AIE2P), f32 inputs.
// Simple elementwise add: out = a + b
// Same strategy as vecadd_aie2p.mlir but pads with an f32 zero instead of a bf16
// zero: @pad_and_promote_binary_bf16 hard-codes a bf16 padding value, which
// aircc rejects when the tensor element type is f32 ("expects a padding value
// of type 'f32', got 0.0 : bf16"). The pad body is inlined here with f32
// padding values (3 operands: 2 inputs + 1 output).
// Unlike ReLU, NO bf16 cast is applied: AIE2 has a legal f32 vector add
// (aievec.add on vector<32xf32>), so the add stays in native f32 and the
// result is bit-accurate.
...
module attributes {transform.with_named_sequence} {
  transform.named_sequence @__transform_main(
    ...
    // f32 binary pad+promote (inlined @pad_and_promote_binary_bf16 with f32 pad
