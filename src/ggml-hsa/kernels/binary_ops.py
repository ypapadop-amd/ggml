...
def apply_binary_op(
    arch: str, input_tensors: list, function: Callable | ExternalFunction, output_tensor
):
    """
    ...
    if isinstance(function, ExternalFunction):
        ...
        worker = Worker(
            ext_core_fn, fn_args=[x.cons() for x in of_ins] + [of_out.prod(), function]
        )
    else:
        # Define a task that will run on a compute tile (Python fallback)
...
def ggml_op_add(arch: str, input_tensors: list, output_tensor, op_params: bytearray):
    ...
    """
    # Use C++ external function for better performance and compatibility
    input_dtype_str = dtype_to_str(np.dtype(input_tensors[0].dtype))
    output_dtype_str = dtype_to_str(np.dtype(output_tensor.dtype))

    ext_fn = ExternalFunction(
        fn_name="ggml_op_add",
        source_file=path.join(path.dirname(__file__), arch, "../binary_ops.cc"),
        num_inputs=2,
        num_outputs=1,
        compile_flags=[
            f"-DCOMPILE_ADD",
            f"-DINPUT_DTYPE={input_dtype_str}",
            f"-DOUTPUT_DTYPE={output_dtype_str}",
        ],
    )
