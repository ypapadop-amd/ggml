...
def align_to_arch(arch: str, size: int, dtype: np.dtype) -> int:
    """
    ...
    Returns:
        int: Aligned size in number of elements.
    """
...
def create_ternary_program(
    arch, op_name, input_tensor, mask_tensor, pos_tensor, output_tensor, scale, max_bias
):
    ...
    def ext_core_fn(of_in, of_mask, of_pos, of_out, function):
        for _ in range_(num_tiles_in):
            ...
            elem_out = of_out.acquire(1)
            function(
                elem_in, elem_mask, elem_pos, elem_out, tile_size_in, scale, max_bias
            )
            of_in.release(1)
            of_mask.release(1)
            of_pos.release(1)
    ...
    worker = Worker(
        ext_core_fn,
        fn_args=[of_in.cons(), of_mask.cons(), of_pos.cons(), of_out.prod(), function],
    )
    ...
    with rt.sequence(
        input_tensor_ty, mask_tensor_ty, pos_tensor_ty, output_tensor_ty
    ) as (a_in, a_mask, a_pos, b_out):
        rt.start(worker)
        rt.fill(of_in.prod(), a_in)
        rt.fill(of_mask.prod(), a_mask)
...
def create_external_function(
    ...
    if pos_tensor is not None:
        row_length_pos, num_rows_pos = get_softmax_dimensions(pos_tensor)
        tile_size_pos = align_to_arch(arch, row_length_pos, pos_tensor.dtype)
        num_elements_pos = tile_size_pos * num_rows_pos
        ...
        result_extra.extend([tile_size_pos, num_rows_pos, num_elements_pos])
