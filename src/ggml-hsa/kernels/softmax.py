...
from build import arch_aligned_num_elements, arch_to_device, max_tile_size, align_to_arch
...
def get_softmax_dimensions(tensor) -> Tuple[int, int]:
    ...
    # GGML shape order: shape[0] = ne00, shape[1] = ne01, etc.
    # ne00 is the softmax dimension (row length)
    # Remaining dimensions are the number of independent rows
...
def ggml_op_softmax(arch: str, input_tensors: list, output_tensor, op_params: bytearray):
    ...
    if input_tensor_count == 1:
        ...
        return create_ternary_program(arch, op_name, input_tensor, mask_tensor, sink_tensor, output_tensor, scale, max_bias)
...
def create_binary_program(arch, op_name, input_tensor, mask_tensor, output_tensor, scale, max_bias):
    ...
    tile_size_mask = func_result[3]      # Was: num_elements_mask (wrong)
    num_rows_mask = func_result[4]       # Was: tile_size_mask (wrong)
    num_elements_mask = func_result[5]   # Was missing
...
def create_ternary_program(arch, op_name, input_tensor, mask_tensor, sink_tensor, output_tensor, scale, max_bias):
    """
    ...
    def ext_core_fn(of_in, of_mask, of_sink, of_out, function):
        # acquire sink array once at the start (not per-tile)
    ...
    with rt.sequence(input_tensor_ty, mask_tensor_ty, sink_tensor_ty, output_tensor_ty) as (a_in, a_mask, a_sink, b_out):
        rt.start(worker)
        rt.fill(of_in.prod(), a_in)
        rt.fill(of_mask.prod(), a_mask)
...
def create_external_function(
    ...
    tile_size_in = align_to_arch(arch, row_length_in, input_tensor.dtype)
    ...
    if mask_tensor is not None:
        ...
        tile_size_mask = align_to_arch(arch, row_length_mask, mask_tensor.dtype)
    ...
    if sink_tensor is not None:
        ...
        # sink array is passed in full, kernel indexes into it
