...
def ggml_op_clamp(arch: str, input_tensors: list, output_tensor, op_params: bytearray):
    ...
    min = struct.unpack_from("f", op_params, 0)[0]
    max = struct.unpack_from("f", op_params, 4)[0]
    ...
    if isinstance(function, ExternalFunction):
        ...
        def ext_core_fn(of_in, of_out, function):
            ...
            for _ in range_(num_tiles):
                ...
                function(elem_in, elem_out, tile_size, min, max)
                of_in.release(1)
                of_out.release(1)
        ...
        def core_fn(of_in, of_out):
            ...
            for _ in range_(num_tiles):
                ...
                for i in range_(tile_size):
                    elem_out[i] = function(elem_in[i], tile_size, min, max)
                of_in.release(1)
                of_out.release(1)
